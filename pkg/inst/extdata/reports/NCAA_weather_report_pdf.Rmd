---
title: \Huge Weather Data Report
...
load_package_database.weather(input.folder = "/Users/liam/Desktop/Crater_data",
                                # system.file("extdata/working_weather",
                                #                     package = "NgoroWeather"),
                              overwrite.db = "yes")
```

```{r jackalhill_damage, include=FALSE}
## Update sf_hyenaR to match new knowledge about jackal hill damage
# sf_hyenaR$weather_stations$end_date[5] <- as.Date("2023-01-11")
```
...
ggplot() +
  ...
  geom_text(data = slice(time_mean, n()), aes(x = time_dbl + 10, y = time_mean, label = "AVG"),
            hjust = 0, family = "Montserrat600", fontface = "bold") +
  labs(title = paste0("Over a 24h period temperature on Ngorongoro Crater floor<br> ranged between ", paste(paste0(round(c(min(daily_range$time_min),
...
### \normalsize Number of rain days (>1mm)

\begin{normalsize}
The below plot shows the number of rain days recorded each month on the floor of Ngorongoro Crater (days with rainfall >1mm) using data collected from `r format(start_date, "%d/%m/%Y")` - `r format(end_date, "%d/%m/%Y")`.
\end{normalsize}
...
summary_data <- input_data |>
  ...
  group_by(site_name_title, date) |> 
  summarise(daily_total = sum(precip), .groups = "drop") |> 
  mutate(month = as.factor(format(date, "%b %Y"))) |> 
  group_by(site_name_title, month) |> 
  summarise(rain_days = sum(daily_total > 1), .groups = "drop") |> 
  group_by(month) |> 
  summarise(se = sd(rain_days),
  ...
  #           month_dbl = first(month_dbl), .groups = "drop") |>
  # mutate(rain_days = round(rain_days, 2),
  #        month = forcats::fct_reorder(month, .x = month_dbl, .desc = FALSE)) |>
  # select(-month_dbl)

ggplot() +
  geom_col(data = summary_data, aes(x = month, y = rain_days),
           colour = "black", width = 0.75,
           position = position_dodge(width = 0.75), alpha = 0.5) +
  geom_errorbar(data = summary_data, aes(x = month, ymin = rain_days - se, ymax = rain_days + se),
                width = 0, linewidth = 0.75,
                position = position_dodge(width = 0.75),
                colour = "grey10") +
  labs(title = paste0("Mean monthly rain days (>1mm) in Ngorongoro Crater ranged between <br>", paste(paste0(round(range(summary_data$rain_days), 0), " days"), collapse = " - ")),
       subtitle = "Grey bars shows monthly montly rain days (>1mm).<br>Errorbars represent standard errors.") +
