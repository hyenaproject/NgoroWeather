...
```{r load_activity_logs, include=FALSE}
weather_station_activity <- create_station_activity.log("../../data/raw")
```
...
input_data_temp <- input_data |> 
  filter(temperature) |>  ## Only when it's an active temperature periods
  ## Filter cases where there are too many NAs
  mutate(date = lubridate::as_date(date_time)) |> 
  group_by(site_name_title, date) |> 
  mutate(nrNA = sum(is.na(air_temp)),
         .groups = "drop") |> 
  filter(nrNA < na_cutoff_daily) |> 
  ungroup()

input_data_rain <- input_data |> 
  filter(rainfall) |>  ## Only when it's an active temperature periods
  ## Filter cases where there are too many NAs
  mutate(date = lubridate::as_date(date_time)) |> 
  group_by(site_name_title, date) |> 
  mutate(nrNA = sum(is.na(air_temp)),
...
station_points <- bind_cols(weather_station_activity$activity_log, sf::st_coordinates(weather_station_activity$activity_log)) |> 
...
\textbf{NOTE:} Months with >7 days of missing data are excluded from the table.
...
daily_mean <- input_data_temp |> 
  #Only deal with stations on crater floor
  filter(site_name != "lemala_rim") |> 
  mutate(date = lubridate::as_date(date_time)) |> 
  group_by(date) |> 
  summarise(daily_mean = mean(air_temp, na.rm = TRUE),
            .groups = "drop") |> 
...
thinned_data <- input_data_temp |> 
  filter(site_name != "lemala_rim") |> 
  mutate(date = lubridate::as_date(date_time)) |> 
  group_by(date) |> 
  summarise(min = min(air_temp, na.rm = TRUE),
            max = max(air_temp, na.rm = TRUE),
            .groups = "drop")
...
ggplot() +
  ...
  scale_x_date(name = "", date_labels = "%d/%m/%Y", limits = c(as.Date(NA), Sys.Date() + 10)) +
  scale_y_continuous(name = "Air temperature (°C)", limits = c(-10, 40), expand = c(0, 0)) +
  coord_cartesian(clip = "off") +
  labs(title = paste0("Temperature on Ngorongoro Crater floor ranged between <br>", paste(paste0(round(range(filter(input_data_temp, site_name != "lemala_rim")$air_temp, na.rm = TRUE), 0), "°C"), collapse = " - ")),
...
possible_dates <- input_data_temp |> 
...
summary_data <- possible_dates |> 
  ...
  filter(sum(is.na(daily_mean)) < na_cutoff_monthly) |> 
...
daily_range <- input_data_temp |> 
  filter(site_name != "lemala_rim") |> 
  mutate(time_cat = format(date_time, "%H:%M"),
         time_dbl = lubridate::hour(date_time)*60 + lubridate::minute(date_time)) |> 
  group_by(time_cat) |> 
  summarise(time_min = min(air_temp, na.rm = TRUE),
            time_max = max(air_temp, na.rm = TRUE),
            time_dbl = first(time_dbl), .groups = "drop")
...
\textbf{NOTE:} Months with >7 days of missing data are excluded.
...
## Create list of possible dates for each station
possible_dates <- input_data_rain |> 
  group_by(site_name_title) |> 
...
daily_data <- input_data_rain |>
  #Only deal with stations on crater floor
  filter(site_name != "lemala_rim") |>
  mutate(date = lubridate::as_date(date_time, tz = "Africa/Dar_es_Salaam")) |> 
  group_by(site_name_title, date) |> 
  summarise(daily_total = sum(precip, na.rm = TRUE),
            .groups = "drop")
...
summary_data <- possible_dates |> 
  ...
  group_by(site_name_title, month) |> 
  ## Filter cases where >=1 week of data is missing
  filter(sum(is.na(daily_total)) < na_cutoff_monthly) |> 
  group_by(month) |> 
...
rain_palette <- scales::col_numeric(c("#FFFFFF", "#DFF6FF", "#5D8BF4", "#2D31FA", "#051367"), domain = c(0, 16), na.color = "#cccccc")
...
ggplot() +
  ...
  geom_errorbar(data = summary_data, aes(x = month, ymin = lower, ymax = upper),
                ...
                position = position_dodge(width = 0.75),
                colour = "grey10") +
  labs(title = paste0("Proportion monthly rain days (>1mm) in Ngorongoro Crater ranged between <br>", paste(paste0(round(range(summary_data$mean), 1), " "), collapse = " - ")),
...
site_data_rain <- input_data_rain |> 
  filter(site_name_title == all_names[1])

## Identify unmonitored periods and separate
site_data_unmonitored <- input_data |> 
  filter(site_name_title == all_names[1] & !rainfall) |> 
  mutate(timediff = c(30, as.numeric(diff(date_time))),
         group = c(1, rep(NA_real_, times = (n() - 1))))

site_data_unmonitored[site_data_unmonitored$timediff > 60, ]$group <- (1:sum(site_data_unmonitored$timediff > 60) + 1)

site_data_unmonitored <- site_data_unmonitored |> 
...
#Determine mean of each day from across both stations
daily_sum <- site_data_rain |> 
  mutate(date = lubridate::as_date(date_time)) |> 
  group_by(date) |> 
  summarise(total_rain = sum(precip, na.rm = TRUE)) |> 
  mutate(date_time = lubridate::ymd_hms(paste(date, "12:00:00"), tz = "Africa/Dar_es_Salaam"))
...
plot_rain_1 <- ggplot() +
  geom_area(data = site_data_unmonitored,
              aes(x = value, y = rain_limit, group = group,
                  fill = "No data collected"),
            alpha = 0.5, colour = NA) +
  ...
  scale_x_datetime(name = "", date_labels = "%d/%m/%Y", limits = c(as.POSIXct(NA), Sys.Date() + 10)) +
  scale_y_continuous(name = "Daily rainfall (mm)", limits = c(0, rain_limit), breaks = seq(0, 125, 25), expand = c(0, 0)) +
  labs(caption = paste0("Data: ", all_names[1], " weather station collected at 30 min intervals")) +
  coord_cartesian(clip = "off") +
  theme_classic() +
...
temp_range <- site_data |>
  mutate(date = lubridate::as_date(date_time)) |>
  group_by(date) |>
  summarise(min_temp = min(air_temp, na.rm = TRUE),
...
## Identify unmonitored periods and separate
site_data_unmonitored <- input_data |> 
  filter(site_name_title == all_names[2] & !rainfall) |> 
  mutate(timediff = c(30, as.numeric(diff(date_time))),
         group = c(1, rep(NA_real_, times = (n() - 1))))
...
## Identify unmonitored periods and separate
site_data_unmonitored <- input_data |> 
  filter(site_name_title == all_names[3] & !rainfall) |> 
  mutate(timediff = c(30, as.numeric(diff(date_time))),
         group = c(1, rep(NA_real_, times = (n() - 1))))
...
## Identify unmonitored periods and separate
site_data_unmonitored <- input_data |> 
  filter(site_name_title == all_names[4] & !rainfall) |> 
  mutate(timediff = c(30, as.numeric(diff(date_time))),
         group = c(1, rep(NA_real_, times = (n() - 1))))
